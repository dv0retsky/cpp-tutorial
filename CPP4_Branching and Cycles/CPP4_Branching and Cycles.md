...
Фрилансер Татьяна устала от разгадывания капчи в интернете, поэтому пошла на завод по получению натуральных чисел. За станком Татьяны есть только 2 кнопки: `-1` и `:2`.  

На экране у Татьяны появляются два числа `A` и `B` (`A > B`).  
Татьяна может разделить число на 2, если оно чётное, либо вычесть 1 в противном случае.  

Напишите программу, которая выводит последовательность нажатий кнопок (каждое нажатие — в отдельной строке),  
